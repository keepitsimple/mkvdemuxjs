...
const mkvdemuxjs = (function(mkvdemuxjs) {
  ...
  MkvDemux.prototype = {
    ...
    peek: function(count) {
      ...
      const q = this.queue[this.peekHi];
      ...
      if (q.byteLength >= this.peekLo + count) {
        const ret = new Uint8Array(q).slice(this.peekLo, this.peekLo + count);
        this.peekLo += count;
        if (this.peekLo >= q.byteLength) {
          this.peekHi++;
          this.peekLo = 0;
      ...
      let pos = q.byteLength - this.peekLo;
      let left = count - pos;
      let i = this.peekHi + 1;
      while (left) {
        ...
        const next = new Uint8Array(this.queue[i++]);
        if (next.length >= left) {
    ...
    commit: function() {
      ...
      if (this.peekLo) {
        ...
        const nq = new Uint8Array(this.queue[0]).slice(this.peekLo);
        if (nq.length === 0)
          this.queue.shift();
        else
    ...
    peekVint: function(keepMarker) {
      ...
      const pre = this.savePeek();
      let header = this.peek(1);
      if (header === null)
        return null;
    ...
    readEBMLHeader: function() {
      ...
      // Update our context
      this.commit();
      const ret = new EBML(id, length, prePos, this.pos);
      if (length >= 0) {
    ...
    readEBMLBody: function() {
      ...
      // Read it in
      const hdr = this.block;
      const len = hdr.end - this.pos;
      const ret = this.peek(len);
      if (ret === null)
        return null;
    ...
    readUInt: function() {
      const len = this.block.length;

      // Read it in
      let val = this.readEBMLBody();
      if (len === 0)
        return 0;
      if (val === null)
        return null;
    ...
    readTrackEntry: function() {
      ...
      while (this.block === te) {
        ...
        case 0x83: // CodecType
          const ct = ret.track.codecType = this.readUInt();
          let t = "unknown";
          switch (ct) {
          case 1:
            t = "video";
    ...
    decodeBlock: function() {
      ...
      // Surrounding data
      const ret = {};
      let clusterTimestamp = this.ex.clusterTimestamp;
      if (!clusterTimestamp)
        clusterTimestamp = 0;

      // Byte 0: Track number
      let track = cont.getUint8(0);
      if (!(track & 0x80))
        console.log("ERROR: Track #s greater than 127 are not supported!");
      track &= 0x7F;

      // Bytes 1 and 2: Timecode offset
      const timeOff = cont.getInt16(1);
      const timestamp = (clusterTimestamp + timeOff) / 1000;

      // Byte 3: Flags
      const flags = cont.getUint8(3);
      if ((flags & 0x6) !== 0)
